...
export class WebhookController {
  ...
  // ─── BUYPOWER MFB WEBHOOK (wallet funding) 
  @Post('buypower')
  @HttpCode(200)
  async handleBuypowerMfb(
    ...
    // Verify signature
    const secret       = this.config.get<string>('BUYPOWER_MFB_WEBHOOK_SECRET');
    ...
    if (isProduction && secret && signature) {
      ...
      this.logger.log('Signature check skipped — dev mode');
    }

    const event       = payload?.event || payload?.type || '';
    const data        = payload?.data  || payload;
    const status      = data?.status   || data?.transactionStatus || '';
    const accountType = data?.accountType || data?.bankAccountType || '';

    this.logger.log(`Event: "${event}", Status: "${status}", Type: "${accountType}"`);

    //  Handle reserved account funding (wallet top-up)
    if (
      event === 'collection.successful'   ||
      event === 'collection.successfully' ||
      event === 'account.credited'        ||
      (status === 'CONFIRMED' && accountType === 'RESERVED') ||
      (status === 'CONFIRMED' && accountType === 'Normal')
    ) {
      return this.handleWalletFunding(payload);
    }
  ...
  private async handleWalletFunding(payload: any) {
    const data = payload?.data || payload;
    ...
    this.logger.log(
      `Wallet funding — nuban: ${nuban}, amount: ₦${amount}, ref: ${reference}`,
    );
    ...
    // Find wallet by NUBAN
    const wallet = await this.walletService.findUserByNubanOrReference(
      nuban     || '',
      reference || '',
    );

    if (!wallet) {
      const all = await this.walletService.getAllWalletNubans();
      this.logger.warn(`Wallet not found. All NUBANs: ${JSON.stringify(all)}`);
      return { received: true, error: 'Wallet not found' };
    }
    ...
    try {
      const result = await this.walletService.creditFromWebhook(
        ...
        reference || `bp-${nuban}-${Date.now()}`,
        {
          ...
          metadata: {
            ...
            reference,
            sourceAccount: data?.sourceAccountNumber,
            sourceName:    data?.sourceAccountName,
            narration:     data?.narration,
          },
        },
      );
      ...
      await this.notification.create({
        ...
        message: `₦${amount.toLocaleString()} has been added to your wallet.`,
        type:    'TRANSACTION',
        metadata: { amount, nuban, reference },
      });
